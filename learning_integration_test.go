...
func TestAgentProposesAndWaitsForProtectedEvaluation(t *testing.T) {
	...
	cfg, count := providerConfigFor(t, func(w http.ResponseWriter, r *http.Request) {
		...
		case last.Content == "Improve":
			functionCompletion(w, "runtime.tool.propose", "propose", draftCommand{Kind: "skill", Description: "Proposed learning", Content: "Use the approved answer", Requires: []string{}})
		case last.Role == "tool":
			var result struct {
	...
	check := daemonSystem(t, d, fixtureControlToken, "POST", base+"/learning/checks", "checks", map[string]any{"cases": []protectedCase{{Input: "protected", Expected: "correct"}}}, 201)
	protectedID.Store(learningResult(t, check, "check_id"))
	goal := "Improve"
	daemonSystem(t, d, fixtureControlToken, "POST", base+"/start", "start", startSystemCommand{ExpectedRevision: 1, Goal: &goal}, 202)
...
func learningResult(t *testing.T, record systemRecord, key string) string {
	t.Helper()
...
func TestProtectedSkillLearningApprovalReuseAndLimits(t *testing.T) {
	...
	check := daemonSystem(t, d, fixtureControlToken, "POST", base+"/learning/checks", "checks", map[string]any{"cases": []protectedCase{{Input: "protected input", Expected: "correct"}}}, 201)
	checkID := learningResult(t, check, "check_id")
	draft := daemonSystem(t, d, fixtureControlToken, "POST", base+"/tools/drafts", "draft", draftCommand{Kind: "skill", Description: "Learned prompt fragment", Content: "Use the approved answer", Requires: []string{}}, 201)
	toolID := learningResult(t, draft, "tool_id")
	evaluate := evaluateCommand{ToolID: toolID, Version: 1, CheckID: checkID, TaskID: waiting.Execution.TaskID}
	...
	daemonSystem(t, d, fixtureControlToken, "POST", base+"/tools/drafts", "next-version", draftCommand{ToolID: toolID, ExpectedVersion: 1, Kind: "skill", Description: "Next approved revision", Content: "Use the approved answer, revised", Requires: []string{}}, 201)
	evaluate.Version = 2
	...
	daemonSystem(t, d, fixtureControlToken, "POST", base+"/tools/drafts", "bad-version", draftCommand{ToolID: toolID, ExpectedVersion: 2, Kind: "skill", Description: "Rejected revision", Content: "An incorrect answer", Requires: []string{}}, 201)
	evaluate.Version = 3
	...
	def.Operator.Tools = def.Tools
	revision := reviseSystemCommand{ExpectedRevision: 1, Configuration: &def, LocalTools: []toolPin{nextPin}}
	...
	goal := "reuse"
	daemonSystem(t, d, fixtureControlToken, "POST", base+"/start", "reuse", startSystemCommand{ExpectedRevision: system.Revision, Goal: &goal}, 202)
	result := waitSystem(t, d, system.ID, func(s systemRecord) bool { return s.State == "inactive" })
	...
	system = daemonSystem(t, d, fixtureControlToken, "PUT", base+"/configuration", "rollback", reviseSystemCommand{ExpectedRevision: system.Revision, Configuration: &def, LocalTools: []toolPin{pin}}, 200)
	daemonSystem(t, d, fixtureControlToken, "POST", base+"/start", "rolled-back", startSystemCommand{ExpectedRevision: system.Revision, Goal: &goal}, 202)
	result = waitSystem(t, d, system.ID, func(s systemRecord) bool { return s.State == "inactive" })
	...
	daemonSystem(t, d, fixtureControlToken, "POST", base+"/start", "exhausted", startSystemCommand{ExpectedRevision: system.Revision, Goal: &goal}, 202)
	result = waitSystem(t, d, system.ID, func(s systemRecord) bool { return s.State == "inactive" })
