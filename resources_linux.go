...
func prepareResourceProcess(cmd *exec.Cmd, profile sandboxConfig) (func() error, error) {
...
func prepareResourceWorkspace(root string, profile sandboxConfig) error {
	if profile.Resources == nil {
		return nil
	}
	if os.Getpid() != 1 {
	...
	if err := writeMessage(os.Stdout, message{Type: "sandbox.workspace"}); err != nil {
		return err
	}
	ack, err := readMessage(bufio.NewReaderSize(os.Stdin, maxFrame+1))
	if err != nil {
		return err
	}
	if ack != (message{Type: "sandbox.workspace.accepted"}) {
