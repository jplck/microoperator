...
func fixtureTeamEngine(t *testing.T, cfg configuration) (*executionEngine, string) {
	t.Helper()
...
func claimAction(t *testing.T, engine *executionEngine, name string, args any) executionRecord {
	t.Helper()
...
func invokeAction(t *testing.T, engine *executionEngine, e executionRecord) string {
	t.Helper()
...
func finishAction(t *testing.T, engine *executionEngine, e executionRecord) {
	t.Helper()
...
func TestRegistryScopeDraftsAndRevocation(t *testing.T) {
	...
	e := claimAction(t, engine, "runtime.agent.propose", proposeAgentArgs{Name: "child", Prompt: "work", Tools: []string{"runtime.text.analyze"}, TokenBudget: 10000})
...
func TestDelegationCannotBorrowRecipientAuthority(t *testing.T) {
	...
	first := claimAction(t, engine, "runtime.agent.propose", proposeAgentArgs{Name: "privileged", Prompt: "recipient", Tools: []string{"runtime.text.analyze", "runtime.agent.propose"}, TokenBudget: 20000})
	...
	second := claimAction(t, engine, "runtime.agent.propose", proposeAgentArgs{Name: "narrow", Prompt: "caller", Tools: []string{"runtime.task.delegate"}, TokenBudget: 15000})
	var caller struct {
		ID string `json:"agent_id"`
	}
	...
	third := claimAction(t, engine, "runtime.task.delegate", delegateArgs{AgentID: caller.ID, Prompt: "Ask for narrow work"})
	invokeAction(t, engine, third)
	finishAction(t, engine, third)
	narrow := claimAction(t, engine, "runtime.task.delegate", delegateArgs{AgentID: recipient.ID, Prompt: "Do not borrow your broader tools"})
	if narrow.AgentID != caller.ID {
		t.Fatal("wrong addressed recipient")
	}
	invokeAction(t, engine, narrow)
	finishAction(t, engine, narrow)
	recipientCall := claimAction(t, engine, "runtime.text.analyze", textArguments{Text: "forbidden"})
...
func TestToolSessionAndSchemaDenials(t *testing.T) {
	...
	e := claimAction(t, engine, "runtime.agent.propose", proposeAgentArgs{Name: "child", Prompt: "work", Tools: []string{}, TokenBudget: 100})
	for _, mutate := range []func(*executionRecord){
		func(e *executionRecord) { e.SystemID = "sys_00000000000000000000000000000000" },
		func(e *executionRecord) { e.ActivationID = "foreign" },
		func(e *executionRecord) { e.EventID = "foreign" },
	} {
		bad := e
		mutate(&bad)
	...
	for _, arguments := range []string{`{}`, `null`, `{"text":null}`, `{"text":42}`, `{"text":"x","path":"/etc/passwd"}`, `{"text":"x","text":"y"}`} {
		if err := validateArguments(schema.Function, arguments); err == nil {
...
func TestWaitingDelegationAndAppliedResultSurviveRecovery(t *testing.T) {
	...
	if err := engine.broker.settle(context.Background(), dispatched, providerResult{Known: true, Input: 11, Output: 7, Text: "durable child result"}, time.Now()); err != nil {
		t.Fatal(err)
	}
...
func TestUnknownToolOutcomeCannotReplay(t *testing.T) {
	...
	e := claimAction(t, engine, "runtime.text.analyze", textArguments{Text: "never redispatch"})
	hash, _, err := jsonDigest(e.Actions[0])
	if err != nil {
		t.Fatal(err)
	}
...
func TestAgentAssignmentPinsAndDirectory(t *testing.T) {
	...
	propose := claimAction(t, engine, "runtime.agent.propose", proposeAgentArgs{Name: "child", Prompt: "work", Tools: []string{"runtime.text.analyze"}, TokenBudget: 10000})
	var child struct {
		ID string `json:"agent_id"`
	}
	if err := json.Unmarshal([]byte(invokeAction(t, engine, propose)), &child); err != nil {
	...
	delegate := claimAction(t, engine, "runtime.task.delegate", delegateArgs{AgentID: child.ID, Prompt: "pinned work"})
	var task struct {
	...
	for _, revoke := range []bool{true, false} {
		t.Run(fmt.Sprint(revoke), func(t *testing.T) {
			...
			if revoke {
				...
				e := claimAction(t, engine, "runtime.text.analyze", textArguments{Text: "denied", Save: true})
				if _, err := engine.invokeTool(context.Background(), e, e.Actions[0]); err == nil {
