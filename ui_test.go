...
func fixtureUI(t *testing.T) (*httptest.Server, *executionEngine) {
	...
	client := uiClient(listener.Addr().String())
	t.Cleanup(client.CloseIdleConnections)
	ui := httptest.NewUnstartedServer(nil)
	ui.Config.Handler = newUIHandler(client, fixtureControlToken, fixtureUIToken, ui.Listener.Addr().String(), engine.logger)
	ui.Start()
	t.Cleanup(ui.Close)
	return ui, engine
}
...
func TestUIRejectsNonLoopbackAndSharedCredential(t *testing.T) {
	if err := runUI(context.Background(), "unused", "0.0.0.0:0", io.Discard, io.Discard); err == nil {
		t.Fatal("UI bound a public address")
	}
	t.Setenv(controlTokenEnv, fixtureControlToken)
	t.Setenv(uiTokenEnv, fixtureControlToken)
	if err := runUI(context.Background(), "unused", "127.0.0.1:0", io.Discard, io.Discard); err == nil {
