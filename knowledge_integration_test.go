...
func TestAgentMemorySurvivesRealRestart(t *testing.T) {
	cfg, count := providerConfigFor(t, func(w http.ResponseWriter, r *http.Request) {
		...
		case goal == "store" && last.Role == "user":
			functionCompletion(w, "runtime.memory.put", "remember", memoryCommand{Scope: "agent", Content: "blue-tag durable fact", Evidence: "fixture observation", Confidence: 90, Artifacts: []string{}, RetentionSeconds: 3600})
		case goal == "store":
			completion(w, "stored")
	...
	goal := "recall"
	daemonSystem(t, restarted, fixtureControlToken, "POST", "/v1/systems/"+record.ID+"/start", "recall", startSystemCommand{ExpectedRevision: 1, Goal: &goal}, 202)
...
func TestPausedTimerSurvivesRealRestart(t *testing.T) {
	...
	daemonSystem(t, d, fixtureControlToken, "POST", "/v1/systems/"+record.ID+"/start", "start", startSystemCommand{ExpectedRevision: 1, LifetimeSeconds: 3600}, 202)
	waitSystem(t, d, record.ID, func(s systemRecord) bool { return s.Execution.TaskState == "waiting" })
	...
	command := scheduleCommand{At: due.Format(time.RFC3339Nano), Content: "timer-payload", Budget: 1}
