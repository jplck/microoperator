...
func TestResourceValidationAndNoFallback(t *testing.T) {
	profile := sandboxConfig{Read: []string{"inputs"}, ReadWrite: []string{"scratch", "output"}, Network: "blocked",
		Resources: &resourceLimits{MemoryBytes: 128 << 20, WorkspaceBytes: 8 << 20, Processes: 64, CPUPercent: 50}}
	if err := validateSandbox("worker", profile); err != nil {
		t.Fatal(err)
	}
	for _, change := range []func(*resourceLimits){
		func(l *resourceLimits) { l.MemoryBytes = 1 }, func(l *resourceLimits) { l.WorkspaceBytes = 0 },
		func(l *resourceLimits) { l.WorkspaceBytes = l.MemoryBytes + 1 }, func(l *resourceLimits) { l.Processes = 1 },
		func(l *resourceLimits) { l.CPUPercent = 0 },
	} {
		bad := profile
		limits := *profile.Resources
		bad.Resources = &limits
		change(bad.Resources)
		if err := validateSandbox("worker", bad); err == nil {
