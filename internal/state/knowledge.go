...
func searchMemory(ctx context.Context, tx *sql.Tx, t TaskRecord, query MemoryQuery, admin bool, now time.Time) (result MemorySearchResult, err error) {
	...
	if len(query.Query) > 256 || len(query.After) > 64 || (!admin && query.Pending) {
		return result, Invalid("memory", "query exceeds scope or size limits")
	}
	...
	for rows.Next() {
		...
		if len(entries) == 10 || size+len(data) > MaxEventBytes-128 {
			if len(entries) == 0 {
				return result, errors.Join(Invalid("memory", "stored entry exceeds retrieval limit"), rows.Close())
			}
	...
	return MemorySearchResult{Untrusted: true, Entries: entries, Next: next}, nil
}
...
func (engine *workflow) knowledgeTool(ctx context.Context, tx *sql.Tx, e ExecutionRecord, t TaskRecord, name, arguments string) (string, error) {
	now := time.Now()
	switch name {
