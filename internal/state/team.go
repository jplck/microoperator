...
func (engine *workflow) claim(ctx context.Context, now time.Time) (record SystemRecord, e ExecutionRecord, found bool, err error) {
...
func (engine *workflow) consumeInputs(ctx context.Context, tx *sql.Tx, t *TaskRecord, exclude string) error {
...
func (engine *workflow) finishDelivery(ctx context.Context, session ExecutionRecord, workerErr error, canceled bool) (err error) {
	tx, err := engine.store.db.BeginTx(ctx, nil)
	if err != nil {
		return err
	}
	...
	shutdown := engine.shuttingDown && systemState != "stopping" && systemState != "stopped" && goalControl != "stopped" && t.Control != "stopped"
...
func (store *Store) RecoverExecutions(ctx context.Context) (err error) {
...
func (engine *workflow) taskSnapshot(ctx context.Context, systemID, taskID string) (t TaskRecord, err error) {
