...
func boundedGeneratedFile(workspace *os.Root, name string) ([]byte, error) {
	...
	if !info.Mode().IsRegular() || info.Size() < 1 || info.Size() > state.MaxGeneratedBinary {
	...
	data, err := io.ReadAll(io.LimitReader(file, state.MaxGeneratedBinary+1))
	err = errors.Join(err, file.Close())
	if len(data) > state.MaxGeneratedBinary {
		err = errors.Join(err, errors.New("generated artifact exceeds size limit"))
	}
	return data, err
}

func BuildGenerated(ctx context.Context, launcher, dataDir, systemID, source string, cfg state.LearningConfig, profile state.SandboxConfig) (binary []byte, err error) {
	if err := state.GeneratedProfile(profile); err != nil {
		return nil, err
	}
	if err := state.ValidateGeneratedSource(source); err != nil {
		return nil, err
	}
	if err := state.ValidateLearningConfig(cfg); err != nil {
		return nil, err
	}
	...
	if digest != cfg.ToolchainDigest {
		return nil, state.Invalid("toolchain", "pinned contents changed")
	}
	...
	err = SuperviseWorkspace(ctx, launcher, root, filepath.Join(cfg.ToolchainRoot, "bin", "go"),
		...
		func(_ io.Writer, out io.Reader) error {
			data, err := io.ReadAll(io.LimitReader(out, state.MaxEventBytes+1))
			if len(data) > state.MaxEventBytes {
...
func RunGenerated(ctx context.Context, launcher, dataDir, systemID string, binary []byte, input string, profile state.SandboxConfig) (output string, err error) {
	if err := state.GeneratedProfile(profile); err != nil {
		return "", err
	}
	if len(binary) == 0 || len(binary) > state.MaxGeneratedBinary || len(input) > 4096 {
		return "", state.Invalid("generated tool", "binary or input exceeds limit")
	}
	...
	err = Supervise(ctx, launcher, root, target, nil, func(in io.Writer, out io.Reader) error {
		...
		result, err := io.ReadAll(io.LimitReader(reader, state.MaxEventBytes+1))
		if err != nil {
			return err
		}
		if len(result) > state.MaxEventBytes {
			return errors.New("generated output exceeds limit")
		}
		var value struct {
			Text string `json:"text"`
		}
		if err := protocol.DecodeJSON(result, &value); err != nil {
			return err
		}
