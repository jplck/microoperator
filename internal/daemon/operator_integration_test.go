...
func TestOperatorModelsStopAndIsolation(t *testing.T) {
	...
	daemonSystem(t, d, fixtureControlToken, "POST", "/v1/systems/"+b.ID+"/start", "start-beta",
		state.StartSystemCommand{ExpectedRevision: 1, Stream: true}, http.StatusAccepted)
	...
	status, _ = daemonRequest(t, d, fixtureControlToken, "PUT", "/v1/systems/"+a.ID+"/configuration", "revise-active",
		state.ReviseSystemCommand{ExpectedRevision: 1, Configuration: &a.Configuration})
	if status != http.StatusConflict {
	...
	close(releaseAlpha)
	doneA := waitSystem(t, d, a.ID, func(s state.SystemRecord) bool { return s.State == "inactive" })
	...
	stopped := waitSystem(t, d, b.ID, func(s state.SystemRecord) bool { return s.State == "stopped" })
	...
	daemonSystem(t, d, fixtureControlToken, "POST", "/v1/systems/"+b.ID+"/start", "gamma",
		state.StartSystemCommand{ExpectedRevision: 1, Goal: &prompt, Stream: true}, http.StatusAccepted)
	...
	close(releaseGamma)
	gamma := waitSystem(t, d, b.ID, func(s state.SystemRecord) bool { return s.State == "inactive" })
...
func TestConfiguredSandboxDoesNotExpandWriteGrants(t *testing.T) {
	...
	profile := state.SandboxConfig{Read: []string{"inputs"}, ReadWrite: []string{"scratch"}, Network: "blocked"}
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()
	var reply protocol.Message
	err = sandbox.Supervise(ctx, microoperatorBinary, root, target, []string{"-sandbox-probe=write", "-sandbox-value=" + filepath.Join(root, "output", "denied")},
