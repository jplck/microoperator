...
func fixtureCall(t *testing.T, b *modelBroker, configID, key string, stream bool) state.ExecutionRecord {
	...
	started, err := b.store.StartSystem(context.Background(), localAdministrator, "start-"+key, record.ID, "fixture-owner",
		state.StartSystemCommand{ExpectedRevision: 1, Stream: stream}, b.cfg, b.now())
	if err != nil {
		t.Fatal(err)
	}
...
func providerConfigFor(t *testing.T, handler http.HandlerFunc) (state.Configuration, *atomic.Int64) {
	t.Helper()
	count := &atomic.Int64{}
...
func admitCall(t *testing.T, b *modelBroker, e state.ExecutionRecord, want bool) state.ExecutionRecord {
	t.Helper()
	b.mu.Lock()
	defer b.mu.Unlock()
...
func dispatchCall(t *testing.T, b *modelBroker, e state.ExecutionRecord) {
	t.Helper()
...
func TestInitialGoalBudgetAndCommandReplay(t *testing.T) {
	...
	revised, err := b.store.ReviseSystem(context.Background(), localAdministrator, "revise", record.ID, state.ReviseSystemCommand{ExpectedRevision: 1, Configuration: &def}, cfg, id)
	if err != nil {
		t.Fatal(err)
	}
	started, err := b.store.StartSystem(context.Background(), localAdministrator, "start", record.ID, "owner", state.StartSystemCommand{ExpectedRevision: 2}, cfg, b.now())
	...
	if _, _, err := b.store.ListCalls(context.Background(), "foreign-owner", record.ID, 0); !errors.Is(err, state.ErrSystemNotFound) {
...
func TestBrokerQueueBoundsAndCancellation(t *testing.T) {
	...
	_, err = b.store.StartSystem(context.Background(), localAdministrator, "start-two", record.ID, "fixture-owner", state.StartSystemCommand{ExpectedRevision: 1}, cfg, b.now())
...
func TestBrokerRejectsOversizedAndImpossibleRequests(t *testing.T) {
	...
	_, err = b.store.StartSystem(context.Background(), localAdministrator, "large-start", record.ID, "owner", state.StartSystemCommand{ExpectedRevision: 1}, cfg, b.now())
