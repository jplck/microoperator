...
func startDaemonFixture(t *testing.T, filename, token string, env ...string) *daemonFixture {
	...
	go func() {
		value, err := protocol.ReadMessage(bufio.NewReaderSize(stdout, protocol.MaxFrame+1))
...
func daemonRequest(t *testing.T, fixture *daemonFixture, token, method, path, key string, body any) (int, []byte) {
	...
	data, readErr := io.ReadAll(io.LimitReader(response.Body, 2*state.MaxConfigBytes))
...
func daemonSystem(t *testing.T, fixture *daemonFixture, token, method, path, key string, body any, wantStatus int) state.SystemRecord {
	t.Helper()
...
func TestRetiredDemoCommandsAreRejected(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			cmd.Env = sandbox.Environment(workspace(t))
...
func TestDaemonStartupDenialsAndExclusiveOwnership(t *testing.T) {
	...
	runFailure([]string{protocol.ControlTokenEnv + "=" + fixtureControlToken}, "api_key_env")
	runFailure([]string{fixtureProviderEnv + "=" + fixtureProviderSecret}, protocol.ControlTokenEnv)
	if _, err := os.Stat(cfg.DataDir); !os.IsNotExist(err) {
		t.Fatalf("failed validation created state: %v", err)
	}
	cfg.SchemaVersion = 99
	writeFixtureConfiguration(t, filename, cfg)
	runFailure([]string{protocol.ControlTokenEnv + "=" + fixtureControlToken, fixtureProviderEnv + "=" + fixtureProviderSecret}, "schema_version")
	cfg.SchemaVersion = 1
	profile := cfg.SandboxProfiles["worker"]
	profile.Network = "allow-all"
	cfg.SandboxProfiles["worker"] = profile
	writeFixtureConfiguration(t, filename, cfg)
	runFailure([]string{protocol.ControlTokenEnv + "=" + fixtureControlToken, fixtureProviderEnv + "=" + fixtureProviderSecret}, "network")
	profile.Network = "blocked"
	cfg.SandboxProfiles["worker"] = profile
	writeFixtureConfiguration(t, filename, cfg)
	daemon := startDaemonFixture(t, filename, fixtureControlToken)
	runFailure([]string{protocol.ControlTokenEnv + "=" + fixtureControlToken, fixtureProviderEnv + "=" + fixtureProviderSecret}, "lock data_dir")
