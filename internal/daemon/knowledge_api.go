...
func (api *controlAPI) memory(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == "POST" {
		var command state.MemoryCommand
	...
	if err != nil {
		api.failure(w, state.Invalid("query", "invalid memory query"))
		return
	}
	for key, values := range query {
		if len(values) != 1 || (key != "scope" && key != "owner_id" && key != "query" && key != "after" && key != "include_pending") {
			api.failure(w, state.Invalid("query", "unknown or repeated memory query field"))
			return
		}
	}
	search := state.MemoryQuery{Scope: query.Get("scope"), Owner: query.Get("owner_id"), Query: query.Get("query"), After: query.Get("after")}
	if search.Scope == "" {
		search.Scope = "system"
	}
	if value := query.Get("include_pending"); value != "" {
		...
		if err != nil {
			api.failure(w, state.Invalid("include_pending", "requires a boolean"))
			return
		}
	}
...
func (api *controlAPI) memoryState(w http.ResponseWriter, r *http.Request) {
	...
	var command state.ChangeMemoryStateCommand
	key, err := readCommand(w, r, &command)
	if err != nil {
		api.failure(w, err)
		return
...
func (api *controlAPI) schedules(w http.ResponseWriter, r *http.Request) {
	...
	var command state.ScheduleCommand
	key, err := readCommand(w, r, &command)
	if err != nil {
		api.failure(w, err)
		return
...
func (api *controlAPI) subscriptions(w http.ResponseWriter, r *http.Request) {
	...
	var command state.SubscriptionCommand
	key, err := readCommand(w, r, &command)
	if err != nil {
		api.failure(w, err)
		return
