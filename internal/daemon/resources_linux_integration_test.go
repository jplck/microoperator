...
func platformResourceProbe(mode, value string) (string, error) {
	...
	case "resource-escape":
		...
		if err := protocol.WriteMessage(os.Stdout, protocol.Message{Type: "escaped", Data: strconv.Itoa(child.Process.Pid)}); err != nil {
			return "", err
		}
		return "", child.Wait()
	case "resource-supervisor":
		...
		defer cancel()
		err = sandbox.Supervise(ctx, args.Launcher, args.Root, args.Target, []string{"-sandbox-probe=resource-escape"}, func(in io.Writer, out io.Reader) error {
			...
			if err := protocol.WriteMessage(os.Stdout, protocol.Message{Type: "resource-group", Data: path}); err != nil {
				return err
			}
...
func resourceTestProfile(group string) state.SandboxConfig {
	return state.SandboxConfig{Read: []string{"inputs"}, ReadWrite: []string{"scratch", "output"}, Network: "blocked",
		Resources: &state.ResourceLimits{MemoryBytes: 256 << 20, WorkspaceBytes: 8 << 20, Processes: 128, CPUPercent: 100}, ResourceRoot: group}
}
...
func TestResourceQualification(t *testing.T) {
	...
	t.Run("private IPC", func(t *testing.T) {
		...
		err := sandbox.Supervise(ctx, microoperatorBinary, workspace(t), target, []string{"-sandbox-probe=resource-shm", "-sandbox-value=" + strconv.FormatUint(uint64(id), 10)},
	...
	for _, tc := range []struct{ mode, want string }{
		...
		t.Run(tc.mode, func(t *testing.T) {
			...
			defer cancel()
			var reply protocol.Message
			err := sandbox.Supervise(ctx, microoperatorBinary, workspace(t), target, []string{"-sandbox-probe=" + tc.mode},
				func(in io.Writer, out io.Reader) error { var err error; reply, err = exchange(in, out); return err }, profile)
			if err != nil {
				t.Fatal(err)
			}
	...
	for _, mode := range []string{"memory", "pids", "cpu"} {
		t.Run(mode, func(t *testing.T) {
			...
			start := time.Now()
			err := sandbox.Supervise(ctx, microoperatorBinary, workspace(t), target, []string{"-sandbox-probe=resource-" + mode}, func(in io.Writer, out io.Reader) error {
				path, err := activeResourceGroup(group)
				if err != nil {
					return err
				}
	...
	t.Run("escaped descendant cancellation", func(t *testing.T) {
		...
		defer cancel()
		path := ""
		err := sandbox.Supervise(ctx, microoperatorBinary, workspace(t), target, []string{"-sandbox-probe=resource-escape"}, func(in io.Writer, out io.Reader) error {
			reply, err := exchange(in, out)
			if err != nil {
				return err
			}
	...
	t.Run("abrupt supervisor death", func(t *testing.T) {
		...
		reader := bufio.NewReaderSize(stdout, protocol.MaxFrame+1)
		ready, err := protocol.ReadMessage(reader)
		if err != nil || ready.Type != "ready" {
			t.Fatalf("supervisor readiness: %+v %v", ready, err)
		}
		if err := protocol.WriteMessage(stdin, protocol.Message{Type: "probe"}); err != nil {
			t.Fatal(err)
		}
		...
		if _, err := sandbox.PrepareResourceRoot(); err != nil {
			t.Fatal(err)
		}
	...
	t.Run("daemon tools and crash recovery", func(t *testing.T) {
		...
		for _, entry := range workspaces {
			if sandbox.WorkspaceName.MatchString(entry.Name()) {
