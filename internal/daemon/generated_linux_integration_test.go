...
func TestGeneratedPromotionAndExactArtifactExecution(t *testing.T) {
	...
	group, err := sandbox.PrepareResourceRoot()
	if err != nil {
		t.Fatal(err)
	}
	...
	digest, err := sandbox.ToolchainDigest(context.Background(), root)
	if err != nil {
		t.Fatal(err)
	}
	cfg.Learning = &state.LearningConfig{ToolchainRoot: root, ToolchainDigest: digest}
	...
	check := daemonSystem(t, d, fixtureControlToken, "POST", base+"/learning/checks", "checks", map[string]any{"cases": []state.ProtectedCase{{Input: "hello", Expected: "HELLO"}}}, 201)
	draft := daemonSystem(t, d, fixtureControlToken, "POST", base+"/tools/drafts", "draft", state.DraftCommand{Kind: "executable", Description: "Uppercase JSON text", Content: `package main; import "strings"; func Process(text string)(string,error){return strings.ToUpper(text),nil}`, Requires: []string{}}, 201)
	toolID := learningResult(t, draft, "tool_id")
	generatedID.Store(toolID)
	job := daemonSystem(t, d, fixtureControlToken, "POST", base+"/learning/evaluate", "evaluate", state.EvaluateCommand{ToolID: toolID, Version: 1, CheckID: learningResult(t, check, "check_id"), TaskID: waiting.Execution.TaskID}, 202)
	...
	pending := daemonSystem(t, d, fixtureControlToken, "POST", base+"/tools/drafts", "cancel-draft", state.DraftCommand{Kind: "executable", Description: "Cancellation fixture", Content: `package main; func Process(text string)(string,error){for{}}`, Requires: []string{}}, 201)
	cancelJob := daemonSystem(t, d, fixtureControlToken, "POST", base+"/learning/evaluate", "cancel-evaluation", state.EvaluateCommand{ToolID: learningResult(t, pending, "tool_id"), Version: 1, CheckID: learningResult(t, check, "check_id"), TaskID: waiting.Execution.TaskID}, 202)
	...
	def.Operator.Tools = def.Tools
	revised := daemonSystem(t, d, fixtureControlToken, "PUT", base+"/configuration", "assign", state.ReviseSystemCommand{ExpectedRevision: 1, Configuration: &def, LocalTools: []state.ToolPin{pin}}, 200)
	goal := "run the approved generated tool"
	daemonSystem(t, d, fixtureControlToken, "POST", base+"/start", "reuse", state.StartSystemCommand{ExpectedRevision: revised.Revision, Goal: &goal}, 202)
	...
	binary, err := state.ReadGeneratedArtifact(cfg.DataDir, system.ID, evaluation.Artifact)
	if err != nil {
		t.Fatal(err)
	}
	path, err := state.GeneratedArtifactPath(cfg.DataDir, system.ID, evaluation.Artifact)
	if err != nil {
		t.Fatal(err)
	}
	...
	daemonSystem(t, d, fixtureControlToken, "POST", base+"/start", "tampered", state.StartSystemCommand{ExpectedRevision: revised.Revision, Goal: &goal}, 202)
	result = waitSystem(t, d, system.ID, func(s state.SystemRecord) bool { return s.State == "inactive" })
	...
	if status, _ := daemonRequest(t, d, fixtureControlToken, "POST", base+"/start", "revoked", state.StartSystemCommand{ExpectedRevision: revised.Revision, Goal: &goal}); status != 400 {
...
func TestGeneratedBuildQualification(t *testing.T) {
	...
	config := state.LearningConfig{ToolchainRoot: root, ToolchainDigest: digest}
	directory := t.TempDir()
...
func Process(s string)(string,error){
 ...
 return strings.ToUpper(s),nil
	...
	binary, err := sandbox.BuildGenerated(ctx, microoperatorBinary, directory, "sys_0123456789abcdef0123456789abcdef", source, config, profile)
	if err != nil {
		t.Fatal(err)
	}
	for _, input := range []string{"hello", secret} {
		output, err := sandbox.RunGenerated(ctx, microoperatorBinary, directory, "sys_0123456789abcdef0123456789abcdef", binary, input, profile)
		if err != nil || output != strings.ToUpper(input) {
			t.Fatalf("generated run: %q %v", output, err)
		}
	}
	config.ToolchainDigest = strings.Repeat("0", 64)
	if _, err := sandbox.BuildGenerated(ctx, microoperatorBinary, directory, "sys_0123456789abcdef0123456789abcdef", source, config, profile); err == nil {
