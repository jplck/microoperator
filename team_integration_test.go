...
func functionCompletion(w http.ResponseWriter, name, id string, args any) {
	...
	data, _ := json.Marshal(map[string]any{
		"choices": []any{map[string]any{"index": 0, "message": map[string]any{"role": "assistant", "content": nil,
			"tool_calls": []any{map[string]any{"id": id, "type": "function", "function": map[string]string{"name": wireToolName(name), "arguments": string(arguments)}}}}, "finish_reason": "tool_calls"}},
...
func TestPausedInputSurvivesRealDaemonRestart(t *testing.T) {
	...
	daemonSystem(t, d, fixtureControlToken, "POST", "/v1/systems/"+record.ID+"/start", "start", startSystemCommand{ExpectedRevision: 1}, 202)
	select {
	case <-entered:
	...
	close(release)
	waitSystem(t, d, record.ID, func(s systemRecord) bool { return s.State == "paused" && s.Execution.State == "awaiting_worker" })
	d.stop(t, true)
	restarted := startDaemonFixture(t, filename, fixtureControlToken)
	state := daemonSystem(t, restarted, fixtureControlToken, "GET", "/v1/systems/"+record.ID, "", nil, 200)
	if state.State != "paused" || count.Load() != 1 {
	...
	done := waitSystem(t, restarted, record.ID, func(s systemRecord) bool { return s.State == "inactive" })
...
func TestSharedToolAndSkillRequireSystemGrant(t *testing.T) {
	cfg, count := providerConfigFor(t, func(w http.ResponseWriter, r *http.Request) {
		...
		functionCompletion(w, "runtime.text.analyze", "analyze", textArguments{Text: "reviewed tool", Save: true})
	})
	cfg.Tools["shared.instructions"] = toolConfig{Kind: "skill", Version: 1, Description: "Pinned instructions", Content: "read-only skill", RequiresTools: []string{"runtime.text.analyze"}}
	...
	for _, launch := range []string{"research", "denied"} {
		...
		daemonSystem(t, d, fixtureControlToken, "POST", "/v1/systems/"+record.ID+"/start", "start-"+launch, startSystemCommand{ExpectedRevision: 1}, 202)
		done := waitSystem(t, d, record.ID, func(s systemRecord) bool { return s.State == "inactive" })
...
func TestGoalStopCancelsWaitingTeamButNotOtherSystem(t *testing.T) {
	...
	cfg, _ := providerConfigFor(t, func(w http.ResponseWriter, r *http.Request) {
		...
		default:
			last := request.Messages[len(request.Messages)-1]
			if last.Role != "tool" {
				functionCompletion(w, "runtime.agent.propose", "propose", proposeAgentArgs{Name: "child", Prompt: "child", Tools: []string{}, TokenBudget: 10000})
				return
			}
			var child struct {
			...
			functionCompletion(w, "runtime.task.delegate", "delegate", delegateArgs{AgentID: child.ID, Prompt: "wait"})
		}
	})
	...
	started := daemonSystem(t, d, fixtureControlToken, "POST", "/v1/systems/"+record.ID+"/start", "start", startSystemCommand{ExpectedRevision: 1}, 202)
	select {
	case <-childEntered:
	...
	daemonSystem(t, d, fixtureControlToken, "POST", "/v1/systems/"+second.ID+"/start", "other-start", startSystemCommand{ExpectedRevision: 1}, 202)
	...
	waitSystem(t, d, record.ID, func(s systemRecord) bool { return s.State == "stopped" })
	done := waitSystem(t, d, second.ID, func(s systemRecord) bool { return s.State == "inactive" })
...
func TestTwoSystemsDelegateThroughDurableMailboxes(t *testing.T) {
	...
	cfg, count := providerConfigFor(t, func(w http.ResponseWriter, r *http.Request) {
		...
		case role == "operator" && lastTool == "":
			functionCompletion(w, "runtime.agent.propose", "propose", proposeAgentArgs{Name: "researcher", Prompt: "child", Tools: []string{"runtime.text.analyze"}, TokenBudget: 20000})
		case role == "operator" && lastFunction == wireToolName("runtime.agent.propose"):
			var result struct {
				ID string `json:"agent_id"`
			}
			...
			functionCompletion(w, "runtime.task.delegate", "delegate", delegateArgs{AgentID: result.ID, Prompt: "child for " + user})
		case role == "child" && lastTool == "":
			if len(request.Tools) != 1 || request.Tools[0].Function.Name != wireToolName("runtime.text.analyze") {
				t.Error("child received broader capabilities")
			}
			functionCompletion(w, "runtime.text.analyze", "analyze", textArguments{Text: user, Save: true})
		case role == "child":
		...
		case role == "operator" && lastFunction == wireToolName("runtime.task.delegate"):
	...
	for _, name := range []string{"alpha", "beta"} {
		...
		records[name] = record
		daemonSystem(t, d, fixtureControlToken, "POST", "/v1/systems/"+record.ID+"/start", "start-"+name, startSystemCommand{ExpectedRevision: 1}, 202)
	}
	for name, record := range records {
		done := waitSystem(t, d, record.ID, func(s systemRecord) bool { return s.State == "inactive" || s.State == "stopped" })
