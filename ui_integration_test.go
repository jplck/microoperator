...
func TestDetachedUIControlsTwoSystemsAndCanExit(t *testing.T) {
	...
	cmd.Env = []string{"LANG=C", "TZ=UTC", controlTokenEnv + "=" + fixtureControlToken, uiTokenEnv + "=" + fixtureUIToken}
	...
	ready, err := readMessage(bufio.NewReaderSize(stdout, maxFrame+1))
	...
	for _, system := range systems.Systems {
		...
		post("/v1/systems/"+system.ID+"/start", "start-"+system.ID, startSystemCommand{ExpectedRevision: 1}, 202)
		waitSystem(t, d, system.ID, func(s systemRecord) bool { return s.Execution.TaskState == "waiting" })
	}
	post("/v1/systems/"+ids["alpha"]+"/tools/drafts", "draft", draftCommand{Kind: "executable", Description: "Inert source", Content: "package main\nfunc main(){}", Requires: []string{}}, 201)
	...
	waitSystem(t, d, ids["alpha"], func(s systemRecord) bool { return s.State == "stopped" })
	...
	close(release)
	result := waitSystem(t, d, ids["beta"], func(s systemRecord) bool { return s.State == "inactive" })
